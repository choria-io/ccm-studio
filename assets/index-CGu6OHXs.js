...
`&&S!=="	"&&(p=f)}if(f>=d)if(p)c.push(p),d=p+l,p=void 0;else if(s===Si){for(;h===" "||h==="	";)h=k,k=e[f+=1],v=!0;const S=f>y+1?f-2:g-1;if(u[S])return e;c.push(S),u[S]=!0,d=S+l,p=void 0}else v=!0}h=k}if(v&&a&&a(),c.length===0)return e;n&&n();let w=e.slice(0,c[0]);for(let k=0;k<c.length;++k){const S=c[k],A=c[k+1]||e.length;S===0?w=`
...
`&&A!=="	"&&A!==" ")break}let h=s.substring(p);const v=h.indexOf(`
